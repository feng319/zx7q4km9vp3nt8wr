...
class TestPromptDictsHaveBothLanguages:
    ...
    def test_relational_prompt(self):
        from chunks2skus.extractors.relational_extractor import RELATIONAL_INCREMENTAL_PROMPT

        assert "en" in RELATIONAL_INCREMENTAL_PROMPT
        assert "zh" in RELATIONAL_INCREMENTAL_PROMPT
    ...
    def test_mapping_prompt(self):
        from chunks2skus.extractors.meta_extractor import MAPPING_DIFF_PROMPT

        assert "en" in MAPPING_DIFF_PROMPT
        assert "zh" in MAPPING_DIFF_PROMPT

    def test_eureka_prompt(self):
        from chunks2skus.extractors.meta_extractor import EUREKA_DIRECT_PROMPT

        assert "en" in EUREKA_DIRECT_PROMPT
        assert "zh" in EUREKA_DIRECT_PROMPT
    ...
    def test_chatbot_prompts(self):
        from skus2ontology.chatbot import (
        ...
        assert "en" in ANCHOR_SYSTEM_PROMPT
        assert "zh" in ANCHOR_SYSTEM_PROMPT
...
class TestPlaceholderParity:
    ...
    def test_relational_prompt_placeholders(self):
        ...
        en_ph = self._extract_placeholders(RELATIONAL_INCREMENTAL_PROMPT["en"])
        zh_ph = self._extract_placeholders(RELATIONAL_INCREMENTAL_PROMPT["zh"])
        assert en_ph == zh_ph, f"Mismatch: en={en_ph}, zh={zh_ph}"
    ...
    def test_mapping_prompt_placeholders(self):
        ...
        en_ph = self._extract_placeholders(MAPPING_DIFF_PROMPT["en"])
        zh_ph = self._extract_placeholders(MAPPING_DIFF_PROMPT["zh"])
        assert en_ph == zh_ph, f"Mismatch: en={en_ph}, zh={zh_ph}"

    def test_eureka_prompt_placeholders(self):
        from chunks2skus.extractors.meta_extractor import EUREKA_DIRECT_PROMPT

        en_ph = self._extract_placeholders(EUREKA_DIRECT_PROMPT["en"])
        zh_ph = self._extract_placeholders(EUREKA_DIRECT_PROMPT["zh"])
        assert en_ph == zh_ph, f"Mismatch: en={en_ph}, zh={zh_ph}"
    ...
    def test_chatbot_system_prompt_placeholders(self):
        from skus2ontology.chatbot import ANCHOR_SYSTEM_PROMPT, SYSTEM_PROMPT_TEMPLATE

        en_ph = self._extract_placeholders(SYSTEM_PROMPT_TEMPLATE["en"])
        zh_ph = self._extract_placeholders(SYSTEM_PROMPT_TEMPLATE["zh"])
        assert en_ph == zh_ph, f"Mismatch: en={en_ph}, zh={zh_ph}"

        en_anchor = self._extract_placeholders(ANCHOR_SYSTEM_PROMPT["en"])
        zh_anchor = self._extract_placeholders(ANCHOR_SYSTEM_PROMPT["zh"])
        assert en_anchor == zh_anchor, f"Mismatch: en={en_anchor}, zh={zh_anchor}"
...
class TestLanguageConfigDefault:
    ...
    def test_module1_default(self):
        ...
        with patch.dict(os.environ, {"LANGUAGE": "en"}, clear=False):
