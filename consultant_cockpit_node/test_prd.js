...
const PORT = parseInt(process.env.PORT || '18501', 10);
...
async function main() {
  ...
  // 创建会话
  let r = await req({ hostname:'localhost', port:PORT, path:'/api/sessions', method:'POST', headers:{'Content-Type':'application/json'} });
  ...
  r = await req({ hostname:'localhost', port:PORT, path:`/api/sessions/${sid}/records`, method:'POST', headers:{'Content-Type':'application/json','Content-Length':Buffer.byteLength(body)} }, body);
  ...
  // 检查 session 状态
  r = await req({ hostname:'localhost', port:PORT, path:`/api/sessions/${sid}`, method:'GET' });
  ...
  r = await req({ hostname:'localhost', port:PORT, path:`/api/sessions/${sid}/records/${recordId}/confirm`, method:'POST', headers:{'Content-Type':'application/json','Content-Length':0} });
  console.log('\n5. /确认 记录:', r.b.success ? '✅' : '❌');
