...
> **创建日期**: 2026-05-02
> **最后更新**: 2026-05-05
> **状态**: 已锁定, Day 1/Day 2 实施完成, UI 审计修复完成, 布局优化完成, 预计算缓存方案更新, 4批次重构完成
...
**核心目标**：`/候选` 指令 1.5 秒内展示结果。

**核心原则**：内容正确 > 响应速度。

#### 3.2.1 后台持续维护"当前最佳候选"缓存
...
| 状态 | 含义 | 触发条件 |
|------|------|----------|
| **Fresh** | 缓存有效，可直接使用 | 预计算完成且未过期 |
| **Stale** | 缓存过期，需重算 | 触发条件变化 |

#### 3.2.4 缓存过期触发条件（四类）

| 触发源 | 具体事件 | 处理方式 |
|--------|----------|----------|
| **共识链变更** | `/确认` 新增已确认事实 | 防抖 10 秒后重算 |
| **共识链变更** | 手动修正共识链 (`source=manual_correction`) | **立即重算**，跳过防抖 |
| **阶段切换** | `/切 商业模式` 或 `/切 行业演示` | **立即重算**，跳过防抖 |
| **SKU 变化** | 备弹区 SKU 可信度状态变化（新增🟢/🟡，或原有🟢/🟡被替换为🔴） | 防抖 10 秒后重算 |

**防抖机制**：
- 连续 `/确认` 只在最后一次的 10 秒后执行一次重算
- 避免顾问快速确认 5 条记录时触发 5 次 LLM 调用
- 防抖计时器可被"立即重算"事件取消

#### 3.2.5 三约束首次满足的特殊处理

**触发场景**：
- 事实数早就够了，但备弹区一直没有🟢/🟡 SKU，直到这一刻才有
- 或者 SKU 早就有了，但事实数刚达到 3 条

**处理逻辑**：
- 检测到三约束**从"不满足"变为"全部满足"**的那一刻
- **取消防抖计时器，立即触发重算**
- 这是整场会议里缓存第一次真正有意义的时刻

**实现要求**：
- 需要追踪**三个约束整体的上一次状态**，不只是事实数
- 在所有四种触发源里统一处理"首次满足"检测
...
> 三约束检查的详细说明见 **3.2.2 三约束检查**。

**产品价值必须显式写出**：三约束未满足时的反向引导本身就是产品价值——让顾问知道"还差什么信息才能做决定"，不能只作为隐含逻辑。
...
- **新记录默认状态为`recorded`**(2026-05-05 重构变更)
- **两条确认路径**:
  1. 直接敲`/确认`: `recorded` → `confirmed`
  2. 候选选中时调用`setCandidateRecordPending()`: `recorded` → `pending_client_confirm`
...
- **target_field 自动提取**:内容以"字段名："开头时自动识别客户档案字段（产品线/客户群体/收入结构/毛利结构/交付情况/资源分布/战略目标/显性诉求/隐性痛点）
...
**左栏状态板结构**（四栏状态板的垂直分区实现）：

| 分区 | 内容 | 数据来源 |
|------|------|----------|
| 字段完整度 | 9个字段的填充状态 | `fields_status` |
| 当前阶段 | 战略梳理/商业模式/行业演示 | `current_stage` |
| 待确认假设 | `status=pending_client_confirm` 的记录 | 共识链过滤 |
...
- **左栏状态板完善**(四栏状态板垂直分区实现):
  - 字段完整度进度条(9字段状态)
  - 当前阶段指示器
  - **待确认假设区**(过滤`status=pending_client_confirm`记录,带确认按钮)
  - 下一步建议(右栏追问建议卡联动)
