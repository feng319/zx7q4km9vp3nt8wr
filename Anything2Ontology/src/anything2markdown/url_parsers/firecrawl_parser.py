...
# Thread-local storage for FireCrawl client.
...
class FireCrawlParser(BaseURLParser):
    ...
    Thread safety: FirecrawlApp client is thread-local for parallel
    processing of multiple URLs.
    """

    parser_name = "firecrawl"

    # Cache model name for URL cache key versioning
    _cache_model_name: str = "firecrawl_v1"
    ...
    def client(self) -> FirecrawlApp:
        """Lazy-load thread-local FireCrawl client."""
        if not hasattr(_thread_local, "client"):
            if not self.api_key:
                raise ValueError("FIRECRAWL_API_KEY not set in environment")
            _thread_local.client = FirecrawlApp(api_key=self.api_key)
        # Ensure the cached client uses the current API key
