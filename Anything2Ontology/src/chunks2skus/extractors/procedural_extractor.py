...
- If the source text contains conditional logic or multi-path choices, they MUST be extracted as decision points in this format:
  - If [condition]: execute [path A]
  - If [condition]: execute [path B]
- If the source text is a purely linear process with no branching, decision points may be omitted — do NOT fabricate conditional branches that do not exist in the source
...
决策点规则：
- 如果原文包含条件判断或多路径选择，必须提炼为决策点，格式如下：
  - 如果 [条件]：执行 [路径 A]
  - 如果 [条件]：执行 [路径 B]
- 如果原文是纯线性流程，无分支逻辑，可以省略决策点部分——不要编造原文中不存在的条件分支
...
class ProceduralExtractor(BaseExtractor):
    ...
    def _create_skill(self, procedure: dict[str, Any], chunk_id: str) -> dict[str, Any] | None:
        ...
        return {
            ...
            "name": name,
            "classification": SKUType.PROCEDURAL,
            "path": f"{self.sku_type.value}/{skill_id}",
            "source_chunk": chunk_id,
            "character_count": char_count,
