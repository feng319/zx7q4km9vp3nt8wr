...
class ProofreadingPostprocessor(BasePostprocessor):
    ...
    # Maximum concurrent SKU scoring workers
    MAX_WORKERS = 6
    ...
    def run(self, **kwargs: Any) -> ConfidenceReport:
        ...
        Uses parallel workers (default: 6) to score SKUs concurrently.
        Thread-safety: Jina client and LLM client are both thread-safe.
        Header file writes are per-SKU so no contention.
        ...
        # Filter to unscored SKUs only
        pending = [s for s in skus if s.confidence is None]
        logger.info(
        ...
        def _score_and_record(sku_entry) -> Optional[tuple]:
            """Score a single SKU and return (entry, sku_entry) tuple."""
            entry = self._score_sku(sku_entry)
            return (entry, sku_entry) if entry else None

        # Parallel scoring
        with ThreadPoolExecutor(max_workers=self.MAX_WORKERS) as executor:
            futures = {
                executor.submit(_score_and_record, sku): sku.sku_id
                for sku in pending
            }
            ...
            for future in as_completed(futures):
                ...
                with report_lock:
                    ...
                    if report.total_scored % 20 == 0:
                        logger.info(
                            "Proofreading progress",
                            scored=report.total_scored,
                            pending=len(pending),
                            avg_confidence=f"{sum(e.confidence for e in report.entries) / len(report.entries):.3f}",
                        )
