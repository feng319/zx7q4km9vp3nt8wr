...
@click.option(
    ...
    "-f",
    is_flag=True,
    help="Force re-process all chunks (reset index, overwrite existing SKUs)",
)
def run(chunks_dir: Path | None, output_dir: Path | None, force: bool):
...
def postprocess_all(skus_dir: Path | None, chunks_dir: Path | None, skip_confidence: bool):
    ...
    cross_doc = results["cross_doc_relations"]
    ...
    click.echo(
        f"  {'跨文档关系' if zh else 'Cross-doc relations'}: "
        f"{cross_doc.total_candidates} {'候选' if zh else 'candidates'}, "
        f"{cross_doc.total_accepted} {'已接受' if zh else 'accepted'}, "
        f"{cross_doc.total_rejected} {'已拒绝' if zh else 'rejected'}"
    )
...
@postprocess.command("cross-relations")
@click.option(
    "--skus-dir",
    "-s",
    type=click.Path(exists=True, path_type=Path),
...
def postprocess_cross_relations(skus_dir: Path | None):
    """Discover cross-document relationships via entity disambiguation and transitive inference."""
    pipeline = PostprocessingPipeline(skus_dir=skus_dir)
    result = pipeline.run_cross_relations()
    zh = settings.language == "zh"
    click.echo(
        f"\n{'跨文档关系发现完成！' if zh else 'Cross-document relation discovery complete!'}"
    )
    click.echo(
        f"  {'候选关系' if zh else 'Candidates'}: {result.total_candidates}"
    )
    click.echo(
        f"  {'已接受' if zh else 'Accepted'}: {result.total_accepted}"
    )
    click.echo(
        f"  {'已拒绝' if zh else 'Rejected'}: {result.total_rejected}"
    )
    click.echo(
        f"  {'关系总数' if zh else 'Total relationships'}: {result.total_existing}"
    )
