...
# Thread-local storage for MarkItDown instances.
# pdfminer/python-pptx are not guaranteed thread-safe, so each thread
# gets its own MarkItDown converter instance.
...
class MarkItDownParser(BaseParser):
    ...
    Thread safety: each thread gets its own MarkItDown instance via
    threading.local() to avoid race conditions in pdfminer/python-pptx.
    """
    ...
    def __init__(self):
        """Initialize the parser (converter is lazily created per-thread)."""
        pass

    @property
    def converter(self) -> MarkItDown:
        """Return a thread-local MarkItDown instance."""
