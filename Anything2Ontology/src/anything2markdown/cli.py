...
@click.option(
    "--no-cache",
    is_flag=True,
    help="Disable URL result cache (force re-fetch)",
)
def run(input_dir: Path | None, output_dir: Path | None, verbose: bool, no_cache: bool):
    """
    ...
    try:
        # Run pipeline
        pipeline = Anything2MarkdownPipeline(no_cache=no_cache)
    ...
    try:
        ...
        if (
            ...
            if not output_content.strip():
                click.echo("Empty output, falling back to PaddleOCR-VL...")
                result.output_path.unlink(missing_ok=True)
                ocr_parser = router.get_ocr_fallback_parser()
                result = ocr_parser.parse(file_path, output)
            elif router.should_fallback_to_ocr(output_content, file_path):
...
def parse_url(url: str, output_dir: Path | None, no_cache: bool):
    """Parse a single URL."""
    # Setup
    setup_logging()
    ...
    if not no_cache and settings.cache_enabled:
        ...
        cached_path, cached_meta = check_cache(url, parser.parser_name, output, model_name)
        if cached_path and cached_meta:
            content = cached_path.read_text(encoding="utf-8")
            click.echo(f"Status: success (cached)")
            click.echo(f"Parser: {parser.parser_name}")
            click.echo(f"Output: {cached_path}")
            click.echo(f"Characters: {len(content)}")
            return

    result = parser.parse(url, output)

    # Save to cache on success
    if result.status == "success" and not no_cache and settings.cache_enabled and result.output_path.exists():
